    ...
    pub fn init() MenuBackoffVoice {
        ...
        return MenuBackoffVoice {
            ...
            .note_tracker = zang.Notes(Instrument.NoteParams).NoteTracker.init([_]SongEvent {
                SongEvent { .params = IParams { .freq = 70.0, .note_on = true }, .note_id = 1, .t = f * 0.0 },
                SongEvent { .params = IParams { .freq = 75.0, .note_on = true }, .note_id = 2, .t = f * 0.01 },
                SongEvent { .params = IParams { .freq = 80.0, .note_on = true }, .note_id = 3, .t = f * 0.02 },
                SongEvent { .params = IParams { .freq = 85.0, .note_on = true }, .note_id = 4, .t = f * 0.03 },
                SongEvent { .params = IParams { .freq = 90.0, .note_on = true }, .note_id = 5, .t = f * 0.04 },
                SongEvent { .params = IParams { .freq = 95.0, .note_on = true }, .note_id = 6, .t = f * 0.05 },
                SongEvent { .params = IParams { .freq = 100.0, .note_on = true }, .note_id = 7, .t = f * 0.06 },
                SongEvent { .params = IParams { .freq = 105.0, .note_on = false }, .note_id = 8, .t = f * 0.07 },
            }),
